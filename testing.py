...
class FFTAnalyzer:
    def __init__(self, num_channels, sampling_rate):
        ...
        print(f"[FFT Setup] Freq Resolution: {self.freq_resolution:.3f} Hz/bin")
    ...
    def compute_fft(self, channel, time_data):
        ...
        # Correct scaling: 
        # 1. Multiply by 2 to account for negative frequencies
        # 2. Divide by window correction factor to compensate for window energy
        fft_magnitude = np.abs(fft_result) * (2.0 / self.window_correction)
        
        # DEBUG: Print detected peak frequency
        if channel == 0:
            peak_idx = np.argmax(fft_magnitude[1:]) + 1   # Skip DC component (0Hz) by starting from index 1
            peak_freq = self.freqs[peak_idx]
            print(f"[FFT] Channel {channel+1}: Peak at {peak_freq:.2f} Hz")
...
class EEGMonitor(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.fft_plot.setYRange(0, 2000)
    ...
    def update_plot(self):
        samples, _ = self.inlet.pull_chunk(timeout=0.0, max_samples=500)
        if samples:
    ...
    def update_brainpower_plot(self):
        ...
        if band_powers is not None:
            ...
            relative_powers = [smoothed_powers['delta'], smoothed_powers['theta'], 
                             smoothed_powers['alpha'], smoothed_powers['beta'], 
                             smoothed_powers['gamma']]

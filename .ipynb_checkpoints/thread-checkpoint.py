...
def read_arduino_data(ser, csv_writer=None):
    ...
    raw_data = ser.read(ser.in_waiting or 64)  # Read available data from the serial buffer
...
def read_data_thread(ser, csv_writer):
    """
    Thread function to continuously read data from Arduino.
    Args: ser (serial.Serial): The serial object connected to Arduino.
          csv_writer (csv.writer, optional): CSV writer object to log data. Defaults to None.
    """
...
def logging_thread():
    """
    Thread function to manage logging intervals.
    """
    ...
    while not exit_flag.is_set():
        ...
        if elapsed_since_last_10_minutes >= 600:  # If 10 minutes have passed
            ...
            last_ten_minute_time = current_time  # Update the last 10-minute start time
...
def parse_data(port, baudrate, lsl_flag=False, csv_flag=False, verbose=False):
    ...
    with serial.Serial(port, baudrate, timeout=0.01) as ser:
        ...
        data_thread = threading.Thread(target=read_data_thread, args=(ser, csv_writer))
        data_thread.start()
